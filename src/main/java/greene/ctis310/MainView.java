...
public class MainView extends Pane {
    ...
    // Button that makes the dog bark and makes a new dog with the name and age from
    // the textfields and the breed and color from the choiceboxes
    ...
    public MainView(Dog dog) throws URISyntaxException {
        ...
        // add a default choice for the choicebox
        breedChoice.setValue("Terrier");
        ...
        // create a button that makes a new dog with the name and age from the
        // textfields and the breed and color from the choiceboxes
        ...
        newDogButton.setOnAction(event -> {
            ...
            try {
                if (nameField.getText().contains("/")) {
                    throw new IncorrectCharException("Error: Character not allowed");
                } else if (nameField.getText().equals("")) {
                    throw new BlankFieldException("Error: Can't be blank");
                } else if (nameField.getText().length() > 15) {
                    throw new NameLengthException("Error: not less than 15 characters");
                }
            } catch (IncorrectCharException | BlankFieldException | NameLengthException e) {
                valid = false;
                nameField.setText("");
                nameField.setPromptText(e.getMessage());
            }
            try {
                if (Integer.parseInt(ageField.getText()) < 0 || Integer.parseInt(ageField.getText()) > 100) {
                    throw new NegativeIntegerException("Error: must be between 0-100");
                } else if (ageField.getText().equals("")) {
                    throw new BlankFieldException("Error: Can't be blank");
                }
            } catch (NumberFormatException | NegativeIntegerException | BlankFieldException e) {
                valid = false;
                ageField.setText("");
                if (e instanceof NumberFormatException) {
                    ageField.setPromptText("Error: Must be an integer");
                } else {
                    ageField.setPromptText(e.getMessage());
                }
            }
            if (valid) {
                this.dog = new Dog(nameField.getText(), Integer.parseInt(ageField.getText()), breedChoice.getValue(),
                        colorChoice.getValue());
            }
            // setting the labels to the new dog's attributes
